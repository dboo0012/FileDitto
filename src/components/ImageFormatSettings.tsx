...
export const ImageFormatSettings: React.FC<ImageFormatSettingsProps> = ({
  selectedFormat,
  onFormatSelect,
  selectedQuality,
  onQualitySelect,
  ...
  // Get image formats that are supported by the backend
  const getAvailableImageFormats = (): ImageFormat[] => {
  ...
  const getFormatDescription = (format: ImageFormat): string => {
    switch (format) {
      case "jpeg":
        return "Best for photos";
      case "png":
        return "Best for graphics";
      case "webp":
  ...
  const availableFormats = getAvailableImageFormats();
      ...
      <div>
        ...
        {files.length > 0 && (
          <div className="mb-3 text-xs text-blue-600 bg-blue-50 p-2 rounded">
            ...
            {
              files.filter(
                (f) => FormatUtils.detectMediaType(f.name) === "image"
              ).length
            }{" "}
            image file(s)
          </div>
      ...
      {files.length > 0 && !selectedFormat && (
        <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
          ...
          <div className="text-sm text-blue-800 space-y-1">
            <div>
              • <strong>JPEG:</strong> Photos
            </div>
            <div>
              • <strong>PNG:</strong> Graphics & logos
            </div>
            <div>
              • <strong>WebP:</strong> Web images
            </div>
          </div>
        </div>
      )}
