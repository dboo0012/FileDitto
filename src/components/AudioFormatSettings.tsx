...
interface AudioFormatSettingsProps {
...
export const AudioFormatSettings: React.FC<AudioFormatSettingsProps> = ({
  selectedFormat,
  onFormatSelect,
  selectedQuality,
  onQualitySelect,
  ...
  // Get audio formats that are supported by the backend
  const getAvailableAudioFormats = (): AudioFormat[] => {
  ...
  const getFormatDescription = (format: AudioFormat): string => {
    switch (format) {
      case "mp3":
        return "Universal compatibility";
      case "aac":
        return "Better quality than MP3";
      case "wav":
        return "Uncompressed, lossless";
      case "flac":
        return "Lossless compression";
      case "ogg":
        return "Open source format";
      case "m4a":
        return "Apple compatible";
      case "opus":
        return "Modern, efficient";
      default:
        return "";
    }
  };
  ...
  const getQualityDescription = (quality: string): string => {
    switch (quality) {
      case "high":
        return "Best quality, larger files";
      case "medium":
        return "Balanced quality and size";
      case "low":
        return "Smaller files, lower quality";
      default:
        return "Standard settings";
    }
  };

  const availableFormats = getAvailableAudioFormats();
      ...
      <div>
        ...
        {files.length > 0 && (
          <div className="mb-3 text-xs text-blue-600 bg-blue-50 p-2 rounded">
            ...
            {
              files.filter(
                (f) => FormatUtils.detectMediaType(f.name) === "audio"
              ).length
            }{" "}
            audio file(s)
          </div>
      ...
      {files.length > 0 && !selectedFormat && (
        <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
          ...
          <div className="text-sm text-blue-800 space-y-1">
            <div>
              • <strong>MP3:</strong> Best compatibility
            </div>
            <div>
              • <strong>FLAC:</strong> Lossless quality
            </div>
            <div>
              • <strong>Opus:</strong> Best efficiency
            </div>
          </div>
        </div>
      )}
